...
/**
 * @description
 * Validator for FormGroup that checks if the fiels are equal.
 *
 * @usageNotes
 * ### Validate that two fields are equal
 * ```typescript
 * const formGroup = new FormGroup({
 *   password: new FormControl('1234569'),
 *   confirmPassword: new FormControl('123456'),
 *   }, [isEqualValidator('password', 'confirmPassword')]);
 *   console.log(formGroup.errors); // { equal: true }
 *   ```
 *
 * @returns A validator function that returns an error with the `equal` property if the validation is not equal, otherwise `null`.
 * @param firstField - The name of the first field.
 * @param secondField - The name of the second field.
 */
export const isEqualValidator = (firstField: string, secondField: string) => (group: AbstractControl): ValidationErrors | null => {
